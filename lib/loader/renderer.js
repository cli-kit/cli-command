...
Renderer.prototype.token = function(token, parser) {
  ...
  else if(this.level === 2 && this.custom
    && this.section !== manual.OPTIONS && this.section !== manual.COMMANDS) {
    if(token.raw && !this.inList) {
      //if(this.section === 'files') {
        //console.log('ADDING CUSTOM SECTION %s', this.section);
        //console.dir(token);
        //console.log('END ADDING CUSTOM SECTION %s', this.section);
      //}
      this.conf.help.sections[this.section] += token.raw;
    }
  }
  ...
  if((this.parent !== this.cli
    ...
    || (this.custom && next && next.type === 'heading' && next.depth === 2) ) {
    this.section = null;
    this.parent = this.cli;
    this.custom = null;
  }
