...
Renderer.prototype.getParentCommand = function(text, parent) {
  ...
  //console.log('testing with command text %s', text);
...
Renderer.prototype.merge = function(key) {
  ...
  var definition = this.current[key][last.key()];
...
Renderer.prototype.addOption = function(text, cmd) {
  ...
  //FIXME: improve this logic so it does not conflict with key prefixes
  ...
  if(this.section === manual.OPTIONS) {
    ...
    cmd.command(name);
    this.parent.last().description(description);
  }
