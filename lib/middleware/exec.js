...
function findExecutableCommand(command, req, next) {
  ...
  // TODO :iterate over exec list and compare to arguments
  for(i = 0;i < raw.length;i++) {
    cmd = raw[i]; arg = alias(cmd, command._commands);
    if(arg) {
      raw.splice(i, 1);
      if(exec[arg.key()]) {
        try {
      ...
      if(Object.keys(arg.commands()).length) {
        //findExecutableCommand.call(this, arg, req, next);
      }
    }
  }
  next();
...
module.exports = function(){
  return function exec(req, next) {
    ...
    findExecutableCommand.call(this, this, req, next);
    //var i, raw = req.result.raw.slice(0), cmd, arg, ps;
    //for(i = 0;i < raw.length;i++) {
      //cmd = raw[i]; arg = alias(cmd, this._commands);
      //if(arg) {
        ////console.log('got exec cmd %s %s', cmd, this._exec[arg.key()]);
        //raw.splice(i, 1);
