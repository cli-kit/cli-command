...
module.exports = function() {
  return function command(req, next) {
    ...
    var scope = this, conf = this.configure();
    ...
    var delimiter = conf.command.delimiter, parts;
    for(i = 0;i < raw.length;i++) {
      cmd = raw[i];
      if(delimiter) {
        if(delimiter instanceof RegExp) {
          console.log('split on command delimiter %s', delimiter);
          if(delimiter.test(cmd)) {
            parts = cmd.split(delimiter);
            console.log('got command delimiter match %j', parts);
          }
        }else if(typeof delimiter === 'string' && ~cmd.indexOf(delimiter)) {
          parts = cmd.split(delimiter);
          cmd = parts[0];
          parts = parts.slice(1);
        }
      ...
      if(arg) {
        ...
        if(typeof action == 'function') {
          ...
          req.command.validate = function(cb) {
    ...
    if(conf && conf.command.required && raw.length) {
