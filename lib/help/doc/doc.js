...
var pad = utils.pad, repeat = utils.repeat, ucfirst = utils.ucfirst;
...
HelpDocument.prototype.description = function(data, stream) {
  ...
  if(this.cli.description() && !data.sections.description) {
    data.sections.description = this.getDescription(this.cli.description());
  }
...
HelpDocument.prototype.examples = function(data, stream) {
  this.render(HelpDocument.EXAMPLES, data, stream);
}
...
HelpDocument.prototype.render = function(key, data, stream) {
  var enabled = this.hasSection(key);
  if(!enabled) return false;
  var section = this.getSection(data, key);
  if(section) {
 ...
 *  @return The synopsis.
 */
HelpDocument.prototype.getSynopsis = function() {
  var parts = [];
