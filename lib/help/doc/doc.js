...
var LINE_ALIGN = 'line';
var FLEX_ALIGN = 'flex';
var align = [COLUMN_ALIGN, LINE_ALIGN, FLEX_ALIGN];
...
var HelpDocument = function(program) {
  ...
  this.space = this.conf.space || repeat();
  this.column = COLUMN;
  this.headers = {};
  this.metrics = {};
  this.getMetrics();
 ...
 *  @param options.space Whitespace used to delimit columns (flex).
 *  @param metrics Options metrics information.
 */
HelpDocument.prototype.align = function(options, metrics) {
  ...
  var rcol = this.metrics.maximum + space.length + indent;
  var diff = this.limit - column;
  // indent amount for description lines
  var lindent = repeat(column + indent);
  if(lines) {
  ...
  if(cols) {
    ...
    left = pad(left, rcol - space.length - indent, true);
  }
  return {
    lead: lead,
    left: left,
...
HelpDocument.prototype.printsection = function(section, stream) {
  ...
  for(var i = 0;i < section.length;i++) {
    ...
    opts.right = this.getDescription(section[i].description || '');
