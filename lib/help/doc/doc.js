...
HelpDocument.prototype.description = function(data, stream) {
  ...
  if(data.description && !data.sections.description) {
    data.sections.description = this.getDescription(data.description);
  }
...
HelpDocument.prototype._options = function(data, stream) {
  var hasOptions = Object.keys(this.cmd._options).length > 0;
...
HelpDocument.prototype.getSection = function(data, key) {
  if(!this.isProgram() && key === HelpDocument.DESCRIPTION) {
    return this.getDescription(this.cmd.description());
...
HelpDocument.prototype.getOptionMetrics = function(keys, target) {
  ...
  for(i = 0;i < keys.length;i++) {
    key = keys[i]
    arg = target[key];
    str = arg.getOptionString(this.delimiter, this.assignment);
 ...
 *  @return The synopsis.
 */
HelpDocument.prototype.getSynopsis = function(wrapped, name, prefix) {
  prefix = prefix || '';
  return auto.call(this, wrapped, name, prefix);
}
