  ...
  if(this.conf.sections) {
    ...
    for(i = 0;i < ckeys.length;i++) {
      ...
      if(!~this.sections.indexOf(k)) {
        // convert section key to lowercase
        this.conf.sections[k] = this.conf.sections[ckeys[i]];
        delete this.conf.sections[ckeys[i]];
...
HelpDocument.prototype.custom = function(data, stream, key) {
  ...
  var text = null;
  if(this.isRootCommand() && this.conf.sections[key]) {
    text = this.conf.sections[key];
  }else if(!this.isRootCommand()
    && this.cmd.sections && this.cmd.sections[key]){
    //console.dir(this.cmd.sections[key]);
    text = this.cmd.sections[key].text;
  }
...
HelpDocument.prototype.write = function(data, stream) {
  ...
  for(i = 0;i < this.sections.length;i++) {
    ...
    k = this.sections[i];
    method = typeof this[k] === 'function' ? this[k] : null;
    args = [data, stream, k];
    if(!method) {
      method = this.custom;
    }
