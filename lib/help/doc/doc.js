 ...
 *
 *  @param doc The document implementation.
 ...
 */
HelpDocument.prototype.align = function(doc, options, metrics) {
  var left = options.left,
    right = options.right,
    indent = options.indent || doc.padding,
    width = options.width,
    wrapr = options.wrap,
    column = options.column || doc.column,
    space = options.space || '';
  column -= indent;
  ...
  if(section) {
    ...
    if(Array.isArray(section)) {
      ...
      for(var i = 0;i < section.length;i++) {
        opts.left = section[i].cmd;
        opts.right = section[i].description;
        //str += doc.align.call(this, doc, opts) + '\n';
        //
        parts.push(doc.align.call(this, doc, opts, true));
      }
      doc.println.call(this, parts, stream, doc);
      //str = str.replace(/\s+$/, '');
    }else if(typeof section === 'string') {
      return doc.indent.call(
        this, section, null, data, stream, doc);
    }
  }
}

HelpDocument.prototype.println = function(parts, stream, doc) {
  var i, part;
 ...
 */
HelpDocument.prototype.getOptionString = function(arg, doc) {
  var conf = this.configure();
