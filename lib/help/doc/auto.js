 ...
 */
function getFlagExpansion(conf) {
  var parts = [];
  if(!conf.expansions) return parts;
  var re = /^-[a-zA-Z0-9]$/;
  var chars = '';
  ...
  for(z in opts) {
    ...
    for(i = 0;i < names.length;i++) {
      if(re.test(names[i])) chars += names[i].substr(1);
    }
  }
  return chars.length ? START + HYPHEN + chars + END : parts;
}
...
module.exports = function auto(wrapped, name, prefix) {
  ...
  // automatic generation (default)
    ...
    parts = parts.concat(getFlagExpansion.call(this, conf));
  }
